    ...
    <Portlet title="案件基本信息">
        <template slot="buttonGroup">
            ...
            <el-button
                    ...
                    size="mini"
                    v-if="recordable"
                    style="float: right;margin-right: 10px;margin-top: 5px">鉴定记录
            </el-button>
            <el-button
                    type="primary"
                    size="mini"
                    v-if="editable"
                    style="float: right;margin-top: 5px">编辑
            </el-button>
        </template>
        <template slot="body">
            <el-row v-cloak>
            ...
            <el-row>
                <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl"><span class="label">鉴定助理</span>
                    <span class="detail" v-if="detailMsg.assist&&detailMsg.assist.participant">{{detailMsg.assist.participant.hrStaff.name}}</span>
                </el-col>
                <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl"><span class="label">第一鉴定人</span>
                    <span class="detail" v-if="detailMsg.auth1&&detailMsg.auth1.participant">{{detailMsg.auth1.participant.hrStaff.name}}</span>
                </el-col>
                <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl"><span class="label">第二鉴定人</span>
                    <span class="detail" v-if="detailMsg.auth2&&detailMsg.auth2.participant">{{detailMsg.auth2.participant.hrStaff.name}}</span>
                </el-col>
            </el-row>
        </template>
    </Portlet>
...
<script>
    ...
    export default {
        name: "CaseDetail",
        props: ['caseId', 'editable', 'recordable'],
