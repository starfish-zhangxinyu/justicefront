<template>
    <div id="form-template">
        <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="委托事项导入" name="import">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            save(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        p.then((res) => {
                            this.$post(api.entrustingMatter+'/import', data).then((res) => {
                                if (res.status == 200) {
                                    this.$message({
                                        message: "导入成功",
                                        type: 'success'
            ...
            dowmTemplete(){
                this.$getFormData(api.template+'/download',{'templeleName':"委托事项导入模板.xlsx"}).then(res=>{
                    debugger;
                    this.download(
                        res.data,
                        "委托事项导入模板.xlsx"
        ...
        watch: {
            activeName: function (newval, oldval) {
                if (newval == 'list') {
                    this.$router.push({path: '/auth/entrustingmatters/list'}).catch(err => {
                        console.log(err)
                    });
                }
                if (newval == 'form') {
                    this.$router.push({path: '/auth/entrustingmatters/form'}).catch(err => {
                        console.log(err)
                    });
                }
            }
