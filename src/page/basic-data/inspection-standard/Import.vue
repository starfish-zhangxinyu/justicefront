<template>
    <div id="form-template">
        <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="检验标准添加" name="form"></el-tab-pane>
            <el-tab-pane label="检验标准导入" name="import">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            save(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        p.then((res) => {
                            this.$post(api.standard+'/import', data).then((res) => {
                                if (res.status == 200) {
                                    this.$message({
                                        message: "导入成功！",
                                        type: 'success'
            ...
            dowmTemplete(){
                this.$getFormData(api.template+'/download',{'templeleName':"检验标准导入模板.xlsx"}).then(res=>{
                    debugger;
                    this.download(
                        res.data,
                        "检验标准导入模板.xlsx"
