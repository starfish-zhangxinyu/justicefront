<template>
    <div id="form-template">
        <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="检验数据类型导入" name="groupImport">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            save(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        p.then((res) => {
                            this.$post(api.authDataGroup+'/import', data).then((res) => {
                                if (res.status == 200) {
                                    this.$message({
                                        message: res.data,
                                        type: 'success'
            ...
            dowmTemplete(){
                this.$getFormData(api.template+'/download',{'templeleName':"检验数据类型导入模板.xlsx"}).then(res=>{
                    this.download(
                        res.data,
                        "检验数据类型导入模板.xlsx"
                    );
        ...
        watch: {
            activeName: function (newval, oldval) {
                ...
                //     this.$router.push({path: '/auth/authDataItem/AuthDataItemList'})
                // } 
