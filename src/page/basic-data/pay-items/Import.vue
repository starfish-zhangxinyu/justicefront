<template>
    <div id="form-template">
        <el-tabs type="border-card" v-model="activeName" @tab-click="handleClick">
            ...
            <el-tab-pane label="收费项目导入" name="import">
                <el-form :model="addForm" size="small" ref="ftForm" :rules="rule" label-width="110px">
                    <el-row>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            save(formName) {
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        p.then((res) => {
                            this.$post(api.chargeItem+'/import', data).then((res) => {
                                if (res.status == 200) {
                                    this.$message({
                                        message: res.data,
                                        type: 'success'
            ...
            dowmTemplete(){
                this.$getFormData(api.template+'/download',{'templeleName':"收费项目导入模板.xlsx"}).then(res=>{
                    debugger;
                    this.download(
                        res.data,
                        "收费项目导入模板.xlsx"
        ...
        watch: {
            activeName: function (newval, oldval) {
                ...
                if (newval == 'form') {
                    this.$router.push({path: '/auth/chargingitems/form'}).catch(err => {
                        console.log(err)
                    });
                }
            }
