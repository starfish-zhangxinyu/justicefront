...
export default {
  ...
  methods: {
    init(res){
      let that = this;
      this.$store.dispatch('acceptCase/changeCNAS', {CNASItemList: res}).then(()=>{
          that.getOrderNumber();
      });
    },
    ...
    confirmImportObj() {
      ...
      var matterName = this.acceptForm.CNASItemList[index].entrustingMatter.name;
      var proType = this.basicMsg.professionCategory.type.value ;
      if (proType =='DNA') {
        ...
        //相同的委托事项不能填写相同的样本
        var isExists = false;
        if (data[index].entrustingMatterTargetList) {
          dnaSampleNames = dnaSampleNames.substr(0, dnaSampleNames.length - 1);
          var key = data[index].entrustingMatter.key.value;
          data.forEach((ev, ei) => {
            var etDnaSampleNames = '';
            data[index].entrustingMatterTargetList.forEach((tv, ti) => {
              etDnaSampleNames += tv.name + ",";
            })
            etDnaSampleNames = etDnaSampleNames.substr(0, etDnaSampleNames.length - 1);
            if (dnaSampleNames == etDnaSampleNames && ev.entrustingMatter.key.value == key) {
              isExists = true;
            }
          })
        }
        if (isExists){
          this.$message.error('所选样本已添加过相同的鉴定事项!');
          return;
        }
