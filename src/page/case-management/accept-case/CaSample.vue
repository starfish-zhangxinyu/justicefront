        ...
        >
          <el-table-column prop="type" width="200%" label="类型">
            <template slot-scope="scope">
              <el-form-item :prop="'caSampleList.' + scope.$index + '.type'">
                <select-from-dict
                  :model.sync="scope.row.type"
                  data_type="ca_sample_type"
                ></select-from-dict>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column prop="sampleNo" width="200%" label="编号">
            <template slot-scope="scope">
              <el-form-item
          ...
          >
            <template slot-scope="scope">
              <el-form-item :prop="'caSampleList.' + scope.$index + '.carrier'">
          ...
          <el-table-column prop="quantity" width="200%" label="数量">
            <template slot-scope="scope">
              <el-form-item
          ...
          <el-table-column
            ...
            width="200%"
            label="品牌"
            v-if="basicMsg.professionCategory.type?basicMsg.professionCategory.type.value == 'ELECTRONIC':false"
          >
            <template slot-scope="scope">
              <el-form-item :prop="'caSampleList.' + scope.$index + '.brand'">
          ...
          >
            <template slot-scope="scope">
              <el-form-item :prop="'caSampleList.' + scope.$index + '.model'">
          ...
          >
            <template slot-scope="scope">
              <el-form-item :prop="'caSampleList.' + scope.$index + '.imeiSn'">
          ...
          >
            <template slot-scope="scope">
              <el-form-item :prop="'caSampleList.' + scope.$index + '.unit'">
                <select-from-dict
          ...
          >
            <template slot-scope="scope">
              ...
              >
                <select-from-dict
                  :model.sync="scope.row.appearanceStatus"
                  data_type="appearance_status"
                ></select-from-dict>
              </el-form-item>
            </template>
          </el-table-column>
          <el-table-column width="150%" prop="handleType" label="处理方式">
            <template slot-scope="scope">
              <el-form-item
...
export default {
            ...
            if (v.entrustingMatterTargetList.length) {
              v.entrustingMatterTargetList.forEach((value, inx) => {
                if (value.name == row.oldName) {
                  this.$store.commit("acceptCase/changeMatterTargetList", {
                    index1: i,
                    index2: inx,
        ...
        if (entrustingMatterList.length) {
          entrustingMatterList.forEach((v, i) => {
            if (v.entrustingMatterTargetList.length) {
              v.entrustingMatterTargetList.forEach((value, inx) => {
                if (value.name == row.oldName) {
                  this.$store.commit("acceptCase/deleteMatterTargetList", {
                    index1: i,
                    index2: inx,
                  });
                }
  ...
  updated() {
    if (
      document.querySelector("#rightBtnBox") &&
      this.acceptForm.caSampleList.length > 0
    ) {
    ...
    this.acceptForm.caSampleList.forEach((v) => {
      v.images = v.imageTemp;
    });
  },
};
</script>
