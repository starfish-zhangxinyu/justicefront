      ...
      <Portlet title="费用减免">
        <template slot="body">
          ...
          >
            ...
            <el-table-column prop="amount.yuanFmt" label="本次减免金额">
            </el-table-column>
            <el-table-column
              label="减免后待收金额"
...
export default {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.$put(api.caseChange + "/audit", data)
            ...
            .catch((err) => {
              this.$message.error(
                err.errMsg.replace("{chargingItemName}", err.extraInfo)
              );
            });
        }
      });
      ...
      columns.forEach((column, index) => {
        ...
        const values = data.map((item) => {
          ...
          if (property == "fiCaseChargeSumByItem.newNotReceived") {
            value =
              Number(value.fiCaseChargeSumByItem.notReceived.cent) -
              Number(value.amount.cent);
          } else {
  ...
  created() {
    this.$store.dispatch("login/headerTitleChange", "费用减免审批");
    ...
    this.$get(api.caseChange + "/get", data)
      .then((res) => {
        if (res.status == 200) {
