<template>
    <div>
        <PageBar title1="鉴定流程" title2="鉴定记录"></PageBar>
        <CaseDetail :caseId="caseId" @getCaseNoFlag="getCaseNoFlag"></CaseDetail>
        <el-form ref="checkForm" label-width="130px" size="small" :model="checkForm" class="demo-ruleForm"
                 label-position="right">
            <Collapse title="检验信息" :active="true" v-if="functionTestInfo">
                <template>
                    <el-row>
                        <el-col :span="11" :offset="0"  v-if="functionTestTime">
                            <Portlet title="检验时间" :is-key="functionTestTime">
                                ...
                                <template slot="body">
                                    <el-table border size="small" :data="checkForm.timeTable" fit style="width:100%">
                                        <el-table-column prop="startDate" label="开始日期">
                                            <template slot-scope="scope">
                                                <el-form-item
                                                        :rules="functionTestTime?$filter_rules({required:functionTestTime}):''"
                                                        label-width="0px"
                                        ...
                                        <el-table-column label="操作" align="center" width="80px">
                                            <template slot-scope="scope">
                                                <el-button type="danger" icon="el-icon-delete" size="mini"
                                                           @click="deleteRow(scope.$index,'timeTable')"></el-button>
                                            </template>
                                        </el-table-column>
                                    </el-table>
                                </template>
                        ...
                        <el-col :span="11" :offset="functionTestTime?2:0"  v-if="functionTestSite">
                            <div class="portlet">
                                <Portlet title="检验地点" :is-key="true">
                                    <template slot="buttonGroup">
                                        <el-button
                                                type="primary"
                                                icon="el-icon-plus"
                                    ...
                                    <template slot="body">
                                        <el-table border size="small" :data="checkForm.addressTable" fit
                                                  style="width:100%">
                                            ...
                                            <el-table-column label="操作" align="center" width="80px">
                                                <template slot-scope="scope">
                                                    <el-button type="danger" icon="el-icon-delete" size="mini"
                                                               @click="deleteRow(scope.$index,'addressTable')"></el-button>
                                                </template>
                                            </el-table-column>
                                        </el-table>
                                    </template>
                    ...
                    <el-row>
                        <el-col :lg="{span:16,offset:4}"  v-if="functionTestEquipment">
                        ...
                        <el-col :lg="{span:16,offset:4}" v-if="functionTestMethod">
        ...
        <!--动态字段-->
        <Dynamicfield ref="dynamicfield" :model.sync="DynamicField"></Dynamicfield>
        <!--酒精检测记录-->
        <AlcoholResult
                ...
                ref="alcoholResult"></AlcoholResult>
        <!--文书车痕等-->
        <AuthResult
        ...
        <DnaResult
                ...
                ref="DnaResult"></DnaResult>
        <!--检测记录-->
        <RecordData :case-id="caseId" :model.sync="addForm.authRecord" :basic-data="authRecordsMsg"  :inspectInfoTestRecord="inspectInfoTestRecord"></RecordData>
        ...
        <Portlet title="检测照片" v-if="testPhoto">
            <template slot="buttonGroup">
                ...
                <!-- <el-button type="primary" icon="el-icon-camera-solid" size="mini"
                        style="float: right;margin-right: 10px;margin-top: 6px">扫描
                </el-button> -->
                <Czur :isScan="true" :model.sync="photoArr"></Czur>
            </template>
            <template slot="body">
                <PictureWall :model.sync="photoArr" :has-title="true">
                </PictureWall>
            </template>
        </Portlet>
        ...
        <el-row>
            <el-col :span="2" :offset="11">
                <el-button type="success" size="mini" class="save-btn" @click="save()"><i class="fa fa-save"></i>
                    保存
                </el-button>
            </el-col>
        </el-row>
...
<script>
    ...
    export default {
        ...
        data() {
            return {
                judgeArr: ['DAMAGE', 'DISABILITY', 'TRACE', 'DOCUMENT', 'VEHICLE'],
                ...
                checkForm: {timeTable: [{}], addressTable: [{}]},//检验信息
                addForm: {
                    authDate: '',
                    authAddress: '',
                    authPhotoList: [],
                ...
                caseId: '',
                DynamicField: {caSysDynamicFieldValueList: []},//动态字段
                //鉴定记录显示字段
        ...
        methods: {
            ...
            getCaseNoFlag(caseNoFlag) {
                ...
                this.$nextTick(() => {
                    this.$refs['alcoholResult'] && this.$refs['alcoholResult'].init(caseNoFlag);
                })
            },
            //获取动态字段
            getDynamicField(condition) {
                ...
                data['page.current'] = 1;
                this.$get(api.dynamicFields, condition).then((res) => {
                    let arr = [].concat(res.data);
                    let finalList = [];
                    arr.forEach((v, i) => {
                        finalList = finalList.concat(v.list);
                    });
                    this.DynamicField.caSysDynamicFieldValueList = finalList.map((v, i) => {
                        if (v.inputType.value == 'SELECT_MULTI' || v.inputType.value == 'CHECKBOX') {
...
//上传照片
            handleChangePhoto(file, fileList) {
                this.photoArr.push({photo: {imageType: this.$getFileType(file), raw: file.raw}, photoName: ''});
            },
//上传附件
            handleChangeAccessory(file, fileList) {
                var fileName = file.name;
                var fileType = fileName.substring(fileName.indexOf('.') + 1, fileName.length + 1);
                this.accessoryArr.push({imageType: fileType, raw: file.raw});
            },
            //最后保存
            async save() {
                ...
                Promise.all(promiseArr).then(function (res) {
                    ...
                    _this.checkForm.timeTable.forEach((v, i) => {
                        if (i + 1 == _this.checkForm.timeTable.length) {
                            authDate += v.startDate + '至' + v.endDate;
                        } else {
                            authDate += v.startDate + '至' + v.endDate + ',';
                        }
                    });
                    ...
                    let p4 = new Promise((resolve, reject) => {
                        _this.$compressImages(_this.accessoryArr).then((res) => {
                            resolve(res);
                        })
                    });
                    Promise.all([p3, p4]).then(res => {
                        _this.addForm.authPhotoList = [].concat(res[0]);
                        _this.addForm.authFileList = [].concat(res[1]);
                        var data = JSON.parse(JSON.stringify(_this.addForm));
                        if (data.authRecordInstrumentList.length) {
                            data.authRecordInstrumentList = data.authRecordInstrumentList.map((v, i) => {
                                return {'instrument': v};
                            })
                        }
                        ;
                        if (data.authRecordStandardList.length) {
                            data.authRecordStandardList = data.authRecordStandardList.map((v, i) => {
                                return {'standard': v};
                            })
                        }
                        ;
                        if (_this.authResultForm.authResultList.length) {
                            data.authResultList = [].concat(_this.authResultForm.authResultList);
                        }
                        ;
                        //毒物数据处理
                        if (_this.poisonForm.poSampleResultList.length) {
                            data.poSampleResultList = [].concat(_this.poisonForm.poSampleResultList);
                        }
                        ;
                        data.authAlcoholResultList = null;
                        //酒精数据处理
                        if(_this.authRecordsMsg.inputContentFlag === '1' && _this.entrustingMatterTypeList.indexOf('ALCOHOL')>-1) {
                            if(_this.alcoholForm.doubleResultList && _this.alcoholForm.doubleResultList.length  && _this.alcoholForm.doubleResultList[0].content1 !== null) {
                                //双通道
                                ...
                                //单通道
                                data.authAlcoholResultList = [].concat(_this.alcoholForm.singleResultList);
                            }
                        }
                        if(_this.entrustingMatterTypeList.indexOf('ALCOHOL')>-1 && _this.authRecordsMsg.inputContentFlag === '0') {
                            //丁叔醇
                            let sample = _this.authRecordsMsg.authAlcoholResultList[0] && _this.authRecordsMsg.authAlcoholResultList[0].sample;
                            let id = _this.authRecordsMsg.authAlcoholResultList[0] && _this.authRecordsMsg.authAlcoholResultList[0].id;

                            data.authAlcoholResultList = [].concat(_this.alcoholForm.tertiaryButanolTable);
                            ...
                            obj.tertiaryButanol1 = data.authAlcoholResultList[0].tertiaryButanol;
                            obj.tertiaryButanol2 = data.authAlcoholResultList[1].tertiaryButanol;
                        ...
                        data.caSysDynamicFieldValueList = [].concat(_this.DynamicField.caSysDynamicFieldValueList);
                        ...
                        const a = _this.$message({
                            ...
                            message: '<span><i class="el-icon-loading" style="font-size: 16px;margin-right: 5px"></i>正在保存中，请稍后</span>',
                            center: true
                        });
                        _this.$post(api.authRecords, data).then((res) => {
                            if (res.status == 200) {
                                a.close();
                                _this.$message({
                                    message: '保存成功！',
        ...
        created() {
            ...
            this.$get(api.dict, dictData).then((res) => {
                this.addressOpt = res.data.list;
            }).catch((err) => {
                console.log(err);
            });
            var pro = this.authRecordsMsg.professionCategory;
            /* debugger */
            //获取专业的显示字段
            ...
            this.$get(api.authRecords + '/' + this.caseId).then((res) => {
                this.authRecordsMsg = JSON.parse(JSON.stringify(res.data));

                this.professionCategoryType = this.authRecordsMsg.professionCategory.type.value;
                    ...
                    if (res.status == 200) {
                        ...
                        if (selectFiledArr) {
                            selectFiledArr.forEach((fv,fi) =>{
                                if (fv =='function.test.time'){this.functionTestTime=true}
                                else if (fv =='function.test.site'){this.functionTestSite=true}
                                else if (fv =='function.test.equipment'){this.functionTestEquipment=true}
                                else if (fv =='function.test.method'){this.functionTestMethod=true}
                                else if (fv =='dna.test.result'){this.dnaTestResult=true}
                                else if (fv =='alcohol.test.record'){this.alcoholTestRecord=true}
                                else if (fv =='poison.test.result'){this.poisonTestResult=true}
                                else if (fv =='inspectInfo.test.record'){this.inspectInfoTestRecord=true}
                                else if (fv =='test.photo') {this.testPhoto=true}
                                else if (fv =='file.inspectInfo.test.record'){this.fileInspectInfoTestRecord=true}
                            });
                        }
                ...
                //酒精检验记录
                if (this.professionCategoryType === 'POISON' && res.data.authAlcoholResultList.length > 0) {
                    if(res.data.authAlcoholResultList[0] && res.data.authAlcoholResultList[0].tertiaryButanol1 && res.data.authAlcoholResultList[0].tertiaryButanol1.length > 0) {
                        //叔丁醇
                        let obj1 =
                            {
                                alcohol:res.data.authAlcoholResultList[0].alcohol1,
                                tertiaryButanol: res.data.authAlcoholResultList[0].tertiaryButanol1,
                            };

                        let obj2 =
                            {
                                alcohol:res.data.authAlcoholResultList[0].alcohol2,
                                tertiaryButanol: res.data.authAlcoholResultList[0].tertiaryButanol2,
                            };
                        this.alcoholForm.tertiaryButanolTable = [obj1, obj2];

                    }else {
                        if (res.data.authAlcoholResultList[0] && res.data.authAlcoholResultList[0].content3) {
                            //双通道时
                            let length = res.data.authAlcoholResultList.length;
                            var doubleResultList1 = [];
                            for (let i = 0; i < length; i++) {
                                const obj1 = Object.assign({}, res.data.authAlcoholResultList[i], {
                                ...
                                const obj2 = Object.assign({}, res.data.authAlcoholResultList[i], {
                            ...
                            this.$set(this.alcoholForm, 'doubleResultList', doubleResultList1);
                            this.path = 'TWO_PATH';
                        } else {
                            this.$set(this.alcoholForm, 'singleResultList', res.data.authAlcoholResultList);
                ...
                // 获取动态字段
                if (!this.authRecordsMsg.caSysDynamicFieldValueList || this.authRecordsMsg.caSysDynamicFieldValueList.length == 0) {
                    ...
                    //处理动态字段
                    this.DynamicField.caSysDynamicFieldValueList = this.authRecordsMsg.caSysDynamicFieldValueList.map((v, i) => {
                        if (v.sysDynamicField.inputType.value == 'SELECT_MULTI' || v.sysDynamicField.inputType.value == 'CHECKBOX') {
                ...
                this.condition = {professionCategoryId: this.authRecordsMsg.professionCategory.id};
                //获取委托事项类型
                ...
                //获取检验信息
                this.addForm.authRecordInstrumentList = this.authRecordsMsg.authRecordInstrumentList.map((v, i) => {
                    return v.instrument;
                });
                this.addForm.authRecordStandardList = this.authRecordsMsg.authRecordStandardList.map((v, i) => {
                    return v.standard;
                });
                ...
                if (this.authRecordsMsg.authDate) {
                    ...
                    if (this.authRecordsMsg.authDate.indexOf(',') > -1) {
                        ...
                        timeArr.forEach((v, i) => {
                            var arr = v.split('至');
                            this.checkForm.timeTable.push({startDate: arr[0], endDate: arr[1]});
                        });
                    } else {
                ...
                if (this.authRecordsMsg.authPhotoList && this.authRecordsMsg.authPhotoList.length) {
                    this.photoArr = [].concat(this.authRecordsMsg.authPhotoList);
                }
                if (this.authRecordsMsg.authFileList && this.authRecordsMsg.authFileList.length) {
                    this.authFileList = [].concat(this.authRecordsMsg.authFileList);
                }
                if (this.authRecordsMsg.authAddress) {
                    ...
                    if (this.authRecordsMsg.authAddress.indexOf(',') > -1) {
                        ...
                        addressArr.forEach((v, i) => {
                            this.checkForm.addressTable.push({address: v});
                        });
                    } else {
                        this.checkForm.addressTable.push({address: this.authRecordsMsg.authAddress});
                    }
                }
