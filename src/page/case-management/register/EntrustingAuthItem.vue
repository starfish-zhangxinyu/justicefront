<template>
  <div>
    <Portlet title="委托鉴定事项" :isKey="true">
      <template
        slot="buttonGroup"
        v-if="this.basicMsg.professionCategory.type.value != 'DNA'"
      >
        <el-button
          type="primary"
...
export default {
  ...
  data() {
    return {
      ...
      page2: { pageSizes: [5, 8, 10], pageSize: 10, total: 0, current: 1 },
      tablesort: null,
      searchdata: {},
          ...
          let p = new Promise(function (resolve, reject) {
            obj.$refs.form.validate((valid) => {
              if (valid) {
                ...
                _this.$message.error("请完善表格中样本信息！");
              }
            });
          });
          return p;
