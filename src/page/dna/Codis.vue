<template>
  <div id="codis">
    <div class="search-box" style="overflow: hidden">
      <el-form label-width="100px" :model="searchForm" size="small">
        <el-row>
          <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl">
            <el-form-item label="案件编号">
              <el-input v-model="searchForm.caseNo" />
            </el-form-item>
          </el-col>
          <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl">
            <el-form-item label="样本编号">
              <el-input v-model="searchForm.sampleNo" />
            </el-form-item>
          </el-col>
          <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl">
            <el-form-item label="检测结果">
              <el-select
                v-model="searchForm.judgeResult"
                placeholder="请选择"
                :clearable="true"
                class="w100"
              >
          ...
          <el-col :sm="panClass.sm" :md="panClass.md" :xl="panClass.xl">
            <el-form-item label="基因突变" prop="mutation" class="margin0">
              <el-select
    ...
    <el-button-group style="margin-bottom: 10px">
      <el-button
        ...
        v-if="hasPermission('dna:codis:importTestResult')"
        @click="importResult"
        >导入实验结果
      </el-button>
      <el-button
        type="primary"
        size="mini"
        v-if="hasPermission('dna:codis:completeTest')"
    ...
    >
      ...
      <el-table-column prop="caseNo" label="案件编号" width="250px">
      ...
      <el-table-column
        prop="dnaResultTemp.analyzeMethod"
        show-overflow-tooltip
        label="分析方法"
      >
      </el-table-column>
      <el-table-column
        prop="dnaResultTemp.createDate"
        show-overflow-tooltip
        label="导入结果时间"
        width="180px"
      >
      ...
      <el-table-column
        prop="dnaResultTemp.judgeResult"
        show-overflow-tooltip
        label="结果"
      >
        <template slot-scope="scope">
          <div
            v-if="
              scope.row.dnaResultTemp.judgeResult &&
              scope.row.dnaResultTemp.judgeResult != '无'
            "
          >
            <span
              >不符合点位数{{ scope.row.dnaResultTemp.mutationCount }}个,{{
                scope.row.dnaResultTemp.entrustingMatterName
              }}结果{{ scope.row.dnaResultTemp.judgeResult }}</span
            >
          </div>
        </template>
      </el-table-column>
    ...
    <div
      ...
      "
    >
      <span style="font-size: 14px; color: #606266">共{{ page.total }}条</span>
      <el-pagination
    ...
    >
      <el-form :model="codisForm" size="small" label-width="0px">
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div
                v-html="$replaceSubTag(scope.row.formula)"
                :style="{ display: scope.row.visibleFormulaHtml }"
              ></div>
              <el-input
                size="small"
                :style="{ display: scope.row.visibleFormulaInput }"
                v-model="scope.row.formula2"
              ></el-input>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <el-button
                ...
                :style="{ display: scope.row.visibleBackFormula }"
                v-if="codisForm.entrustingMatterKey != 'NOCALCULATE'"
              ></el-button>
              <el-button
                type="primary"
                ...
                :style="{ display: scope.row.visibleSaveFormula }"
                v-if="codisForm.entrustingMatterKey != 'NOCALCULATE'"
              ></el-button>
              <el-button
                type="warning"
                ...
                :style="{ display: scope.row.visibleUpdateFormula }"
                v-if="codisForm.entrustingMatterKey != 'NOCALCULATE'"
              ></el-button>
              <el-button
                type="danger"
      ...
      <div slot="footer" class="dialog-footer" ref="codisDetailFooter">
        <el-button @click="codisDetailDialog = false" size="mini" type="primary"
          >取 消</el-button
        >
        <el-button type="primary" size="mini" @click="saveResult"
          >保存</el-button
        >
      </div>
    </el-dialog>
    ...
    >
      ...
      <el-table
        ...
        size="small"
        style="width: 100%"
        @select-all="handleMerge"
        @select="handleMerge"
        :row-class-name="historyTableRowJudgeResultColor"
      >
        <el-table-column type="selection" width="55px"> </el-table-column>
        <el-table-column
        ...
        >
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="checkRow(scope.row)"
              >查看</el-button
            >
            <el-button type="primary" size="mini" @click="setResult(scope.row)"
              >设置为实验结果</el-button
            >
          </template>
        </el-table-column>
    ...
    <el-dialog
      title="基因座详情记录"
      :visible.sync="codisDetailDialog1"
      width="80%"
    >
      <el-form :model="codisForm" size="small" label-width="0px">
        <el-table
        ...
        >
          <el-col :span="6" style="margin-top: 5px">
            <div style="width: 100%">
              ...
              <span
                style="color: red; height: 30px; line-height: 30px; float: left"
                :style="{
                  marginTop:
      ...
      <div slot="footer" class="dialog-footer">
        <el-button
          @click="codisDetailDialog1 = false"
          size="mini"
          type="primary"
          >取 消</el-button
        >
    ...
    <el-dialog
      title="导入错误信息"
      :visible.sync="importExceptionDialog"
      width="40%"
    >
      <el-table
        :data="importDnaResultException"
        fit
        border
        size="small"
        style="width: 100%"
      ...
      >
        <el-table-column type="index" label="序号"></el-table-column>
        <el-table-column property="sampleNo" label="样本编号"></el-table-column>
        <el-table-column
          property="markerName"
          label="基因座名称"
        ></el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      spanArr: [],
      pos: "",
      importExceptionDialog: false, //导入错误信息
      importDnaResultException: [], //导入错误信息
    };
  },
  ...
  methods: {
    ...
    search() {
      this.init(this.page.current, this.page.pageSize);
    },
    handleCheckBox(selection) {
      this.completeDetectionArr = [].concat(selection);
    },
    //完成检测
    completeDetection() {
      var data = [];
      var flag = true;
      var str = "";
      if (this.completeDetectionArr.length == 0) {
        this.$message.error("请选择案件！");
        return;
      }
      this.completeDetectionArr.forEach((v, i) => {
        v.dnaResultDtoList.forEach((v2, i2) => {
          if (!v2.judgeResult) {
            flag = false;
          }
        });
      ...
      if (!flag) {
        this.$message.error("所选案件中未确认检测结果！");
        return;
      }
      this.$post(api.completeDetection, data)
        .then((res) => {
          if (res.status == 200) {
            this.$message({
              message: "已完成检测！",
    ...
    confirmResult(index, row) {
      this.deleteStr = "";
      this.tableHead = [];
      this.$get(api.currentCodis + "/" + row.dnaResultTemp.id)
        .then((res) => {
          if (res.data) {
            ...
            data.dnaCalcDtoList.map((v) => {
              v.formula2 = "";
              //修改公式的显示，隐藏
              v.visibleFormulaHtml = "";
              v.visibleFormulaInput = "none";
              v.visibleUpdateFormula = "";
              v.visibleSaveFormula = "none";
              v.visibleBackFormula = "none";
              return v;
            });
            this.codisForm = data;
    ...
    updateRow(row) {
      row.formula2 = row.formula;
      row.formula2 = row.formula2
        .replace("&", "")
        .replace("@", "L")
        .replace("#", "");
      row.visibleUpdateFormula = "none";
      row.visibleFormulaHtml = "none";
      row.visibleFormulaInput = "";
      row.visibleSaveFormula = "";
      row.visibleBackFormula = "";
    },
    backRow(row) {
      row.visibleUpdateFormula = "";
      row.visibleFormulaHtml = "";
      row.visibleFormulaInput = "none";
      row.visibleSaveFormula = "none";
      row.visibleBackFormula = "none";
    },
    saveRow(row) {
      var json = {};
      json.id = row.id;
      json.markerName = row.markerName;
      json.formula = row.formula2;
      this.$post(api.updateFormula, json)
        .then((res) => {
          var data = JSON.parse(JSON.stringify(res.data));
          data.dnaCalcDtoList.map((v) => {
            v.formula2 = "";
    ...
    handleRecount() {
      ...
      arr.forEach((v, i) => {
        v.dnaSampleDtoList.forEach((item, index) => {
          ...
          dnaTestResultIdAlleleArrStr +=
            dto.id + ":" + dto.allele1 + "," + dto.allele2 + "|";
        });
      });
      dnaTestResultIdAlleleArrStr = dnaTestResultIdAlleleArrStr.substr(
        0,
        dnaTestResultIdAlleleArrStr.length - 1
      );
      var json = {};
      ...
      json.deleteMarkerName = this.deleteStr.substr(
        0,
        this.deleteStr.length - 1
      );
      this.$post(api.recount, json)
        .then((res) => {
          var data = JSON.parse(JSON.stringify(res.data));
          if (!data.judgeResult) {
            data.judgeResult = "肯定";
    ...
    saveResult() {
      ...
      data.deleteMarkerName = this.deleteStr.substr(
        0,
        this.deleteStr.length - 1
      );
    ...
    historyResult(index, row) {
      ...
      this.$post(api.historyCodis, {
        entrustingMatterId: row.dnaResultTemp.entrustingMatterId,
      })
        .then((res) => {
    ...
    setResult(row) {
      ...
      this.$post(api.updateCodis, data)
        .then((res) => {
          if (res.status == 200) {
            this.$message({
              message: "设置成功!",
    ...
    mergeLoca() {
      ...
      var data = [];
      if (this.mergeLocaArr.length < 2) {
        this.$message.error("至少选择2条实验记录！");
        return;
      }
      var frequencyTypeId = "";
      ...
      this.$post(api.mergeCodis, data)
        .then((res) => {
          ...
          if (res.status == 200) {
            ...
            this.$post(api.historyCodis, {
              entrustingMatterId:
                this.historyResultList[index].entrustingMatterId,
            })
              .then((res) => {
                this.dnaResultName = res.data[0].dnaResultName;
                this.historyResultList = res.data;
    ...
    handleSizeChange(pageSize) {
      ...
      this.init(this.page.current, pageSize);
    },
    //当前页码发生变化
    handleCurrentChange(current) {
      this.page.current = current;
      this.init(current, this.page.pageSize);
    },
    //导入实验结果
    importResult() {
    ...
    handleChange(file, fileList) {
      this.$refs.importResultForm.validate((valid) => {
        if (valid) {
          ...
          data.fileType = fileName.substring(
            fileName.indexOf(".") + 1,
            fileName.length + 1
          );
          this.$getBase64(file.raw)
            .then((res) => {
              data.fileBase64 = res;
              this.$post(api.disposeImportCodis, data)
                .then((res) => {
                  if (res.data == "FEW") {
                    this.$confirmBox(
                      "文件中的基因座比试剂盒中的基因座少！是否继续导入？"
                    )
                      .then((res) => {
                        this.importCodisRequest(data);
                      })
                      .catch((err) => {
    ...
    importCodisRequest(data) {
      const loading = this.$loading({
        lock: true,
        text: "正在导入，请稍后...",
        spinner: "el-icon-loading",
      });
      this.$post(api.importCodis, data)
        .then((res) => {
          loading.close();
          if (res.status == 200) {
            this.$message({
            ...
            this.$get(api.finalCodis, data)
              .then((res) => {
                ...
                this.getSpanArr(this.splitTableData);
                this.page.total = res.data.total;
                this.importResultForm = {};
              })
              .catch((err) => {
                console.log(err);
              });
            this.importResultDialog = false;
            if (res.data.importDnaResultException.length > 0) {
              this.importDnaResultException = res.data.importDnaResultException;
              this.importExceptionDialog = true;
            }
          }
        })
        .catch((err) => {
          loading.close();
          if (err.errCode == "message.160001") {
    ...
    tableRowJudgeResultColor({ row }) {
      if (row.dnaResultTemp.judgeResult == "肯定") {
        return "affirm-row";
      } else if (row.dnaResultTemp.judgeResult == "否定") {
        return "negate-row";
      } else if (row.dnaResultTemp.judgeResult == "无") {
        return "without-row";
      }
    },
    historyTableRowJudgeResultColor({ row }) {
    ...
    initTableData(data) {
      ...
      data.forEach((v, i) => {
        //发票切分
        if (v.dnaResultDtoList.length) {
          var invoiceTotalAmount = { cent: 0 };
          v.dnaResultDtoList.forEach((value) => {
            var json = JSON.parse(JSON.stringify(v));
            json.dnaResultTemp = value;
  ...
  created() {
    this.$store.dispatch("login/headerTitleChange", "Codis分析");
    this.tableHeight = window.innerHeight - commonNum.tableBottom + "px";
    this.$get(api.finalCodis, { "page.pageSize": 30, "page.current": 1 })
      .then((res) => {
        this.tableData = [].concat(res.data.list);
        this.initTableData(this.tableData);
        this.getSpanArr(this.splitTableData);
