...
export default {
  ...
  created() {
    if (this.basicData) {
      ...
      /*if (professionCategoryType.value == 'TRACE') { //痕迹鉴定
                   /!* this.judgeData = {title: '痕迹鉴定结果', dataType: 'trace_conclusion'};//专业类别*!/
                    this.authResultForm.authResultList.forEach((v,i)=>{
                        datatype[""+i] = 'trace_conclusion';
                    })
                    this.judgeData = {title: '痕迹鉴定结果'};//专业类别
                }*/
      if (professionCategoryType.value == "DOCUMENT") {
        //文书鉴定
        /* this.judgeData = {title: '文书鉴定结果', dataType: 'document_conclusion'};*/ //专业类别
        this.authResultForm.authResultList.forEach((v, i) => {
