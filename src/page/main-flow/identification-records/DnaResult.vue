<template>
    <div  v-if="this.dnaTestResult">
        ...
        <el-dialog title="基因座详情记录" :visible.sync="codisDetailDialog" width="80%">
            <el-form :model="codisForm" size="small" label-width="0px">
                <el-table :data="codisForm.dnaCalcDtoList" fit border size="small" style="width: 100%"
                    ...
                    <el-table-column v-for="(item,i) in tableHead" :label="item" align="center">
                    ...
                    <el-table-column property="formula" label="公式" v-if="codisForm.entrustingMatterKey!='NOCALCULATE'">
                        <template slot-scope="scope">
                            <div v-html="$replaceSubTag(scope.row.formula)"
                                 :style="{display: scope.row.visibleFormulaHtml}"></div>
                            <el-input size="small" :style="{display:scope.row.visibleFormulaInput}"
                                      v-model="scope.row.formula2"></el-input>
                        </template>
                    </el-table-column>
                    <el-table-column property="address" label="操作" align="center" width="200px">
                        <template slot-scope="scope">
                            <el-button type="primary" size="mini" icon="el-icon-back"
                                       @click="backRow(scope.row)"
                                       :style="{display: scope.row.visibleBackFormula}"
                                       v-if="codisForm.entrustingMatterKey!='NOCALCULATE'"></el-button>
                            <el-button type="primary" size="mini" icon="el-icon-s-order"
                ...
                <el-row style="margin-top: 10px" v-if="codisForm.entrustingMatterKey!='NOCALCULATE'">
                    ...
                    <el-col :span="8">
                        <el-button type="primary" size="mini" icon="el-icon-s-operation" @click="handleRecount" style="float:right;margin-top:5px;">重新计算
                        </el-button>
                    </el-col>
                </el-row>
            </el-form>
        ...
        <!--导入错误信息-->
        <el-dialog title="导入错误信息" :visible.sync="importExceptionDialog" width="40%">
            <el-table :data="importDnaResultException" fit border size="small" style="width: 100%;" max-height="400">
                <el-table-column type="index" label="序号"></el-table-column>
                <el-table-column property="sampleNo" label="样本编号"></el-table-column>
                <el-table-column property="markerName" label="基因座名称"></el-table-column>
...
<script>
    ...
    export default {
        ...
        props: ['model', 'caseId', 'basicData','dnaTestResult'],
        data() {
            return {
        ...
        methods: {
            ...
            updateRow(row) {
                ...
                row.formula2 = row.formula2.replace("&", "").replace("@", "L").replace("#", "");
            ...
            saveRow(row) {
                ...
                this.$post(api.updateFormula, json).then((res) => {
                    var data = JSON.parse(JSON.stringify(res.data));
                    data.dnaCalcDtoList.map(v => {
                        v.formula2 = '';
                        //修改公式的显示，隐藏
            ...
            handleRecount() {
                ...
                json.deleteMarkerName = this.deleteStr.substr(0, this.deleteStr.length - 1);
                ...
                this.$refs.importResultForm.validate((valid) => {
                    if (valid) {
                        ...
                        this.$getBase64(file.raw).then((res) => {
                            data.fileBase64 = res;
                            this.$post(api.disposeImportCodis, data).then((res) => {
                                if (res.data == 'FEW') {
                                    this.$confirmBox('文件中的基因座比试剂盒中的基因座少！是否继续导入？').then((res) => {
                                        this.importCodisRequest(data)
                                    }).catch((err) => {
                                        this.importResultDialog = false;
                                    });
                                } else if (res.data == 'MORE') {
                                    this.$confirmBox('文件中的基因座比试剂盒中的基因座多！是否继续导入？').then((res) => {
                                        this.importCodisRequest(data)
                                    }).catch((err) => {
                                        this.importResultDialog = false;
                                    });
